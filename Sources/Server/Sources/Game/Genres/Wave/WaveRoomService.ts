...
export default class WaveRoomService {
    private waveRooms: Array<WaveRoom> = new Array();
    ...
    public joinPublicWaveRoom(userData: UserData, biome: Biome): WaveRoomPlayerId | false {
        if (!this.canUserDataJoinable(userData)) {
            return false;
        }

        this.leaveCurrentWaveRoom(userData);

        const waveRoom = this.findPublicRoom(biome);
    ...
    public joinWaveRoom(userData: UserData, code: WaveRoomCode): WaveRoomPlayerId | false {
        if (!this.canUserDataJoinable(userData)) return false;
    ...
    private findPublicRoom(biome: Biome): WaveRoom | undefined {
        ...
        return this.waveRooms.find(room => room.biome === biome && isPublic(room)) ||
            this.waveRooms.find(isPublic);
    }
     ...
     */
    private canUserDataJoinable(userData: UserData): boolean {
        return !(
            userData && 
            userData.staticPlayerData &&
            // Determinate multiple rooms using conn
            new Set(this.waveRooms.map(waveRoom => waveRoom.roomCandidates.map(c => c.ws)).flat()).has(userData.staticPlayerData.ws)
        );
    }
}