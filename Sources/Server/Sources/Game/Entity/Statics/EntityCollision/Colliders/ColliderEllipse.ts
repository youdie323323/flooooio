...
import type { Circle, Ellipse, Evolute, Point } from "./Collider";

// https://qiita.com/NaokiHori/items/daf3fd191d51a7e682f8

export default class ColliderEllipse extends AbstractCollider<Ellipse> {
    private shiftAndRotate(disp: Point, angle: number, p0: Point): Point {
    ...
    private rotateAndShift(disp: Point, angle: number, p0: Point): Point {
    ...
    private computeEvolute({ a, b }: Ellipse, t: number): Evolute {
    ...
    private findNormalT(e: Ellipse, p: Point): number {
        ...
        for (; ;) {
            ...
            const evolute = this.computeEvolute(e, t);
    ...
    private fitCircles(e0: Ellipse, e1: Ellipse): [Circle, Circle] {
        ...
        for (let n = 0; n < 10; n++) {
            ...
            const p1_ = this.shiftAndRotate(disp0, angle0, p1);

            // Forward coordinate transform
            const disp1: Point = { x: -e1.x, y: -e1.y };
            const angle1 = -e1.theta;
            const p0_ = this.shiftAndRotate(disp1, angle1, p0);

            // Find optimum parameter t for each ellipse
            const e0_t = this.findNormalT(e0, p1_);
            const e1_t = this.findNormalT(e1, p0_);

            // Compute evolutes and their radii
            evolute0_ = this.computeEvolute(e0, e0_t);
            evolute1_ = this.computeEvolute(e1, e1_t);

            // Backward coordinate transform
            const disp0b: Point = { x: +e0.x, y: +e0.y };
            const angle0b = +e0.theta;
            p0 = this.rotateAndShift(disp0b, angle0b, evolute0_);

            // Backward coordinate transform
            const disp1b: Point = { x: +e1.x, y: +e1.y };
            const angle1b = +e1.theta;
            p1 = this.rotateAndShift(disp1b, angle1b, evolute1_);
        }
    ...
    public computeDelta(e0: Ellipse, e1: Ellipse): number {
        // Get the fitted circles for both ellipses
        const [circle0, circle1] = this.fitCircles(e0, e1);
