...
func (wp *WavePool) broadcastChatReceivPacket(wPId EntityId, chatMsg string) {
	buf := make(
		[]byte,
		1+ // Opcode size
			4+ // Player id size
			(len(chatMsg)+1), // Length + null terminator
	)
	at := 0
	...
	binary.LittleEndian.PutUint32(buf[at:], wPId)
	at += 4

	{ // Write chat message
		copy(buf[at:], []byte(chatMsg))
		at += len(chatMsg)
