...
// Curve data which used for spider leg
...
export default class Mob extends Entity {
    ...
    draw(ctx: CanvasRenderingContext2D) {
        ...
        switch (this.type) {
            ...
            case MobType.Spider: {
                ...
                // Draw legs
                {
                    for (let i = 0; i < curves.length; i++) {
                        ...
                        ctx.save();
                        ctx.rotate(curve.dir * Math.sin(this.moveCounter + i) * 0.15);
                        ctx.beginPath();
                        ctx.moveTo(...curve.start);
                        ctx.quadraticCurveTo(...curve.curve);
                        ctx.strokeStyle = this.getSkinColor("#323032");
                        ctx.lineWidth = 10;
                        ctx.lineCap = "round";
                        ctx.stroke();
                        ctx.restore();
                ...
                ctx.fillStyle = this.getSkinColor("#4f412e");
            ...
            case MobType.CentipedeDesert:
            case MobType.CentipedeEvil: {
                scale = this.size / 40;
                ...
                if (this.type === MobType.CentipedeDesert) {
                    bodyColor = "#d3c66d";
                } else if (this.type === MobType.CentipedeEvil) {
                    bodyColor = "#8f5db0";
                } else {
                    bodyColor = "#8ac255";
                }
