...
export default class Networking {
    ...
    constructor(public ws: WebSocket) {
        ...
        ws.onmessage = (event) => {
            ...
            switch (data.getUint8(offset++)) {
                case ClientBound.WaveSelfId: {
                    waveSelfId = data.getUint32(offset);

                    break;
                }
                case ClientBound.WaveRoomSelfId: {
                    waveRoomSelfId = data.getUint32(offset);

                    break;
                }
                case ClientBound.WaveUpdate: {
                ...
                case ClientBound.WaveRoomUpdate: {
                    if (uiCtx.currentCtx instanceof UserInterfaceTitle) {
                        ...
                        uiCtx.currentCtx.waveRoomPlayers = playerInformations;
                ...
                case ClientBound.WaveRoomJoinFailed: {
                    if (uiCtx.currentCtx instanceof UserInterfaceTitle) {
    ...
    public sendChangeMove(angle: number, magnitude: number) {
        ...
        const data = new Uint8Array([ServerBound.WaveChangeMove, normalizedAngle, Math.round(magnitude * 255)]);
        this.ws.send(data);
    }

    public sendChangeMood(flag: number) {
        const data = new Uint8Array([ServerBound.WaveChangeMood, flag]);
        this.ws.send(data);
    }

    public sendSwapPetal(index: number) {
        const data = new Uint8Array([ServerBound.WaveSwapPetal, index]);
        this.ws.send(data);
    }

    public sendChat(chatMsg: string) {
        const data = new Uint8Array([ServerBound.WaveChat, chatMsg.length, ...this.textEncoder.encode(chatMsg)]);
        this.ws.send(data);
    }

    public sendLeave() {
        const data = new Uint8Array([ServerBound.WaveLeave]);
        this.ws.send(data);
    }
    ...
    public sendRoomJoin(code: string) {
        const data = new Uint8Array([ServerBound.WaveRoomJoin, code.length, ...this.textEncoder.encode(code)]);
        this.ws.send(data);
    }
    ...
    public sendRoomChangeReady(state: WaveRoomPlayerReadyState) {
        const data = new Uint8Array([ServerBound.WaveRoomChangeReady, state]);
        this.ws.send(data);
    }

    public sendRoomChangeVisible(state: WaveRoomVisibleState) {
        const data = new Uint8Array([ServerBound.WaveRoomChangeVisible, state]);
        this.ws.send(data);
    }

    public sendRoomChangeName(name: string) {
        const data = new Uint8Array([ServerBound.WaveRoomChangeName, name.length, ...this.textEncoder.encode(name)]);
        this.ws.send(data);
    }

    public sendRoomLeave() {
        const data = new Uint8Array([ServerBound.WaveRoomLeave]);
        this.ws.send(data);
    }
}