...
pub const ObjectId = u32; // Generation + Index

const PackedObject = packed struct(ObjectId) {
...
pub fn Objects(comptime Object: type, comptime search_field: meta.FieldEnum(Object)) type {
    ...
    return struct {
        ...
        const ConstOwnObjectPtr = *const @This();

        pub const Slice = struct {
            index: Index,
            objs: ConstOwnObjectPtr,
        ...
        /// Returns living objects count.
        pub fn liveCount(objs: *const @This()) usize {
            const dead = &objs.internal.dead;
            const data = &objs.internal.data;

            return data.len - dead.count();
        }

        pub fn slice(objs: *const @This()) Slice {
            return .{
                .index = 0,
                .objs = objs,
            };
        ...
        /// situation).
        fn validateAndUnpack(objs: *const @This(), id: ObjectId, comptime src: std.builtin.SourceLocation) PackedObject {
            const dead = &objs.internal.dead;
            const generation = &objs.internal.generation;

            // TODO(object): decide whether to disable safety checks like this in some conditions,
            // e.g. in release builds
            const unpacked: PackedObject = @bitCast(id);

            if (unpacked.generation != generation.items[unpacked.index])
                @panic(comptime (src.fn_name ++ " called with a dead object (use after delete, recycled slot)"));

            if (dead.isSet(unpacked.index))
                @panic(comptime (src.fn_name ++ " called with a dead object (use after delete)"));
