...
fn tryFuzz() callconv(.c) void {
    ...
    switch (packet_type) {
        0 => {
            ...
            fuzzing_ws.ctx.out.sendWaveChangeMove(angle, magnitude) catch {};
        },
        1 => {
            // Random mood
            const mood = random.intRangeAtMost(u8, 0, 255);

            fuzzing_ws.ctx.out.sendWaveChangeMood(mood) catch {};
        },
        2 => {
            // Random petal swap
            const index = random.intRangeAtMost(u8, 0, 7);

            fuzzing_ws.ctx.out.sendWaveSwapPetal(index) catch {};
        },
        3 => {
            // Random chat
            ...
            fuzzing_ws.ctx.out.sendWaveChat(msg) catch {};
        },
        4 => {
            // Random room create
            const biome = random.intRangeAtMost(u8, 0, 255);

            fuzzing_ws.ctx.out.sendWaveRoomCreate(biome) catch {};
        },
        5 => {
            // Random room join
            ...
            fuzzing_ws.ctx.out.sendWaveRoomJoin(code) catch {};
        },
        6 => {
            // Random ready state
            const state = random.intRangeAtMost(u8, 0, 1);

            fuzzing_ws.ctx.out.sendWaveRoomChangeReady(state) catch {};
        },
        7 => {
            // Random visibility
            const visible = random.intRangeAtMost(u8, 0, 1);

            fuzzing_ws.ctx.out.sendWaveRoomChangeVisible(visible) catch {};
        },
        8 => {
            // Random leave
            if (random.boolean()) {
                fuzzing_ws.ctx.out.sendWaveLeave() catch {};
            } else {
                fuzzing_ws.ctx.out.sendWaveRoomLeave() catch {};
            }
        },
        else => unreachable,
    }
