...
fn toRgb(frgb: FloatingRgb) Rgb {
    return @intFromFloat(frgb);
}

/// Converts u8 rgb to f32 rgb.
fn toFloatingRgb(rgb: Rgb) FloatingRgb {
...
pub fn init(rgb: Rgb) Color {
    return .{ .rgb = rgb };
}

fn mulSafe(self: Color, comptime strength: FloatingRgb) Rgb {
...
pub fn nColorInterpolate(colors: []const Color, t: f32) Color {
    ...
    const index_usize: u8 = @intFromFloat(index);
...
fn comptimeIsValidHexColorCode(comptime code: HexColorCode) bool {
    comptime {
        // Checking length is redundant here since we already constrained length with type
...
const minimum_required_rgb_str_len = "rgb(0,0,0)".len;

fn comptimeIsValidRgbString(comptime str: []const u8) bool {
    comptime {
        return str.len >= minimum_required_rgb_str_len and mem.startsWith(u8, str, "rgb(") and str[str.len - 1] == ')';
    }
}
