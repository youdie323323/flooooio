...
import { HEAPU16 } from "../../../Application";
...
interface ContextLostRestoreMethod {
    // Context lost
    Jj: () => void;
    // Context restore
    Kj: () => void;
...
declare global {
    interface HTMLCanvasElement extends ContextLostRestoreMethod { }
    interface OffscreenCanvas extends ContextLostRestoreMethod { }
}
...
export const createContextApiPseudoModule = ((...[, {
    decodeString,
    decodeCString,
    ...
    return {
        ...
        moduleImports: {
            ...
            1: (ptr: number, alpha: Uint1): number => {
                const canvas = document.getElementById(decodeCString(ptr)) as HTMLCanvasElement | null;
                if (!canvas) return -1;
            ...
            41: (contextId: number, ptr: number, len: number): void => {
                contexts[contextId].textAlign =
                    decodeString(ptr, len) as CanvasTextAlign;
            },
            ...
            57: (contextId: number, wAddr: number, hAddr: number) => {
                ...
                HEAPU16[wAddr >> 1] = canvas.width;
                HEAPU16[hAddr >> 1] = canvas.height;
            },
            ...
            65: (callbackPtr: number) => requestAnimationFrame(getWebAssemblyFunction(callbackPtr)),
        },
