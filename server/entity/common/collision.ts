...
function shiftAndRotate(disp: Point, angle: number, p0: Point): Point {
...
function rotateAndShift(disp: Point, angle: number, p0: Point): Point {
...
function computeEvolute(e: Ellipse, t: number): Evolute {
...
function fitCircles(e0: Ellipse, e1: Ellipse): [Circle, Circle] {
    ...
    for (let n = 0; n < 10; n++) {
        ...
        const p1_ = shiftAndRotate(disp0, angle0, p1);

        // Forward coordinate transform
        const disp1: Point = { x: -e1.x, y: -e1.y };
        const angle1 = -e1.theta;
        const p0_ = shiftAndRotate(disp1, angle1, p0);

        // Find optimum parameter t for each ellipse
        const e0_t = findNormalT(e0, p1_);
        const e1_t = findNormalT(e1, p0_);

        // Compute evolutes and their radii
        evolute0_ = computeEvolute(e0, e0_t);
        evolute1_ = computeEvolute(e1, e1_t);

        // Backward coordinate transform
        const disp0b: Point = { x: +e0.x, y: +e0.y };
        const angle0b = +e0.theta;
        p0 = rotateAndShift(disp0b, angle0b, evolute0_);

        // Backward coordinate transform
        const disp1b: Point = { x: +e1.x, y: +e1.y };
        const angle1b = +e1.theta;
        p1 = rotateAndShift(disp1b, angle1b, evolute1_);
    }

    const evolute0: Circle = { x: p0.x, y: p0.y, r: evolute0_.r };
    const evolute1: Circle = { x: p1.x, y: p1.y, r: evolute1_.r };
